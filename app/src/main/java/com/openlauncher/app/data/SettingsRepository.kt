...
class SettingsRepository(private val context: Context) {
    ...
    private object Keys {
        ...
        val AUTO_REROUTE =
            booleanPreferencesKey("auto_reroute")

        val ROUTE_REFRESH_INTERVAL_SECONDS =
            intPreferencesKey("route_refresh_interval_seconds")

        val OFF_ROUTE_THRESHOLD_METERS =
            intPreferencesKey("off_route_threshold_meters")

        val REROUTE_DELAY_SECONDS =
            intPreferencesKey("reroute_delay_seconds")

        val MINIMUM_ROUTE_GAIN_SECONDS =
            intPreferencesKey("minimum_route_gain_seconds")
    }
    ...
    private fun readSettings(prefs: Preferences): AppSettings {
        ...
        return AppSettings(
            ...
            clockStyle = prefs[Keys.CLOCK_STYLE]?.let { runCatching { ClockStyle.valueOf(it) }.getOrNull() }
                ?: defaults.clockStyle,
            unitSystem = prefs[Keys.UNIT_SYSTEM]?.let { runCatching { UnitSystem.valueOf(it) }.getOrNull() }
                ?: defaults.unitSystem,
            appFont = prefs[Keys.APP_FONT]?.let { runCatching { AppFont.valueOf(it) }.getOrNull() }
                ?: defaults.appFont,
            ...
            sidebarPosition = prefs[Keys.SIDEBAR_POSITION]?.let {
                runCatching {
                    SidebarPosition.valueOf(
                        it
                    )
                }.getOrNull()
            }
                ?: if (prefs[Keys.RIGHT_HAND_DRIVE] == true) SidebarPosition.RIGHT else defaults.sidebarPosition,
            bottomBarShortcutsRight = prefs[Keys.BOTTOM_BAR_SHORTCUTS_RIGHT]
                ?: defaults.bottomBarShortcutsRight,
            dayNightMode = prefs[Keys.DAY_NIGHT_MODE]?.let { runCatching { DayNightMode.valueOf(it) }.getOrNull() }
                ?: defaults.dayNightMode,
            ...
            soundboardPads = prefs[Keys.SOUNDBOARD_PADS_JSON]?.let {
                runCatching {
                    gson.fromJson<List<SoundPadConfig>>(
                        it,
                        object : com.google.gson.reflect.TypeToken<List<SoundPadConfig>>() {}.type
                    )
                }.getOrNull()
            } ?: defaults.soundboardPads,
            vitalsAsBars = prefs[Keys.VITALS_AS_BARS] ?: defaults.vitalsAsBars,
            speedometerDigitalOnly = prefs[Keys.SPEEDOMETER_DIGITAL_ONLY]
                ?: defaults.speedometerDigitalOnly,
            gradientDirection = prefs[Keys.GRADIENT_DIRECTION]?.let {
                runCatching {
            ...
            useCustomBackgroundColor = prefs[Keys.USE_CUSTOM_BG_COLOR]
                ?: defaults.useCustomBackgroundColor,
            autoReroute = prefs[Keys.AUTO_REROUTE] ?: defaults.autoReroute,

            routeRefreshIntervalSeconds = prefs[Keys.ROUTE_REFRESH_INTERVAL_SECONDS]
                ?: defaults.routeRefreshIntervalSeconds,

            offRouteThresholdMeters = prefs[Keys.OFF_ROUTE_THRESHOLD_METERS]
                ?: defaults.offRouteThresholdMeters,

            rerouteDelaySeconds = prefs[Keys.REROUTE_DELAY_SECONDS] ?: defaults.rerouteDelaySeconds,

            minimumRouteGainSeconds = prefs[Keys.MINIMUM_ROUTE_GAIN_SECONDS]
                ?: defaults.minimumRouteGainSeconds
        )
    }
    ...
    private fun writeSettings(prefs: MutablePreferences, s: AppSettings) {
        ...
        prefs[Keys.AUTO_REROUTE] = s.autoReroute
        prefs[Keys.ROUTE_REFRESH_INTERVAL_SECONDS] = s.routeRefreshIntervalSeconds
        prefs[Keys.OFF_ROUTE_THRESHOLD_METERS] = s.offRouteThresholdMeters
        prefs[Keys.REROUTE_DELAY_SECONDS] = s.rerouteDelaySeconds
        prefs[Keys.MINIMUM_ROUTE_GAIN_SECONDS] = s.minimumRouteGainSeconds
    }
